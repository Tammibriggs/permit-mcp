...
permit_server = PermitServer(
    mcp, exclude_tools=['validate_resource', 'create_access_request', 'create_operation_approval', 'list_resources', 'list_resources_instances'])
...
def permitio_tools_guide() -> str:
    ...
    return """# Tool Use Parameter Values
- resource: restaurants
- resource_instance (required): This parameter is required and should **ALWAYS** be supplied during tool use, else there will be an error for this use case. Its value can be obtained by first using the `list_restaurants` tool.

Note: *ALWAYS* supply the `resource_instance` parameter.
"""
